...
/**
 * Items FieldType Constructor
 * @extends Field
 * @api public
 */
function items (list, path, options) {
	this._nativeType = [String];
	this._underscoreMethods = ['format'];
	this.separator = options.separator || ' | ';
	items.super_.call(this, list, path, options);
}
items.properName = 'Items';
util.inherits(items, FieldType);

/**
 * Formats the field value
 */
items.prototype.format = function (item, separator) {
 ...
 */
items.prototype.addFilterToQuery = function (filter) {
	var query = {};
 ...
 */
items.prototype.validateInput = function (data, callback) {
	...
	if (value !== undefined && value !== null && value !== '') {
		...
		for (var i = 0; i < value.length; i++) {
			...
			if (Object.keys(thisValue).length !== 3) {
				console.log('Object.keys(thisValue).lengt: ', Object.keys(thisValue).length);
				console.error('hm')
				// result = false;
				// break;
			}
 ...
 */
items.prototype.validateRequiredInput = function (item, data, callback) {
	var value = this.getValueFromData(data);
	var result = false;
 ...
 * Deprecated
 */
items.prototype.inputIsValid = function (data, required, item) {
	var value = this.getValueFromData(data);
	if (required) {
 ...
 */
items.prototype.updateItem = function (item, data, callback) {
	var value = this.getValueFromData(data);
	if (value === undefined || value === null || value === '') {
		value = [];
	}
