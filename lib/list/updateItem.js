...
function updateItem (item, data, options, callback) {
	...
	keys.forEach(key => {
		if (key.indexOf('soap') > -1) {
			var prop = key.split('$i')[1].split('$')[1];
			console.log('prop: ', prop);
			pryps[prop].push(data[key]);
			console.log('pryps[prop]: ', pryps[prop]);
			console.log('data[key]: ', data[key]);
		}
	});

	console.log('pryps.title: ', pryps.title);
	console.log('pryps.title.length: ', pryps.title.length);
	var ttty = pryps.title.map((t, i) => {
		return `${t}§§${pryps.description[i]}§§${pryps.price[i]}`;
	});
	console.log('ttty: ', ttty);

	data['pameldingsinfo.felt'] = ttty;
